...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        # Create a pie chart with counts of successful launches by site
        fig = px.pie(filtered_df, names='Launch Site', title='Total Success Launches by Site', hole=0.3)
        return fig
    else:
        # Filter dataframe for the selected site and group by 'class'
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site].groupby('class').size().reset_index(name='count')
        # Create a pie chart with success vs failed counts for the selected site
        fig = px.pie(filtered_df, values='count', names='class', title=f'Total Success Launches for Site {entered_site}')
        return fig
